...
public class danknet : Script
{
    ...
    private void OpenTrainerMenu()
    {
        ...
        button = new MenuButton("IPL Loader Menu", "TODO:EDIT THIS");
    ...
    private void OpenYanktonMenu()
    {
        ...
        menuItems.Add(button);
        button = new MenuButton("TP to North Yankton", "");
        button.Activated += (sender, args) => this.tptony(Game.Player.Character);
        menuItems.Add(button);
        button = new MenuButton("Load Burnt Farm", "");
        button.Activated += (sender, args) => this.loadburntfarm();
        menuItems.Add(button);
        button = new MenuButton("Load Normal Farm", "");
        button.Activated += (sender, args) => this.loadfarm();
        menuItems.Add(button);
        button = new MenuButton("TP to Farm", "");
        button.Activated += (sender, args) => this.Telep(new Vector3(2441.2f, 4968.5f, 51.7f));
        menuItems.Add(button);
        button = new MenuButton("Load Cluckin Bell", "");
        button.Activated += (sender, args) => this.loadcluckingbell();
        menuItems.Add(button);
        button = new MenuButton("TP to Cluckin Bell", "");
        button.Activated += (sender, args) => this.Telep(new Vector3(-72.68752f, 6253.72656f, 31.08991f));
        menuItems.Add(button);
        button = new MenuButton("Load Destroyed Hospital", "");
        button.Activated += (sender, args) => this.loaddesthospital();
        menuItems.Add(button);
        button = new MenuButton("Load Normal Hospital", "");
        button.Activated += (sender, args) => this.loadnormalhospital();
        menuItems.Add(button);
        button = new MenuButton("TP to Hospital", "");
        button.Activated += (sender, args) => this.Telep(new Vector3(356.8f, -590.1f, 43.3f));
        menuItems.Add(button);
        button = new MenuButton("Load Plane Crash", "");
        button.Activated += (sender, args) => this.loadplanecrash();
        menuItems.Add(button);
        button = new MenuButton("TP to Plane Crash", "");
        button.Activated += (sender, args) => this.Telep(new Vector3(2814.7f, 4758.5f, 50.0f));
        menuItems.Add(button);
        button = new MenuButton("Load Stilt House", "");
        button.Activated += (sender, args) => this.loadstilthouse();
        menuItems.Add(button);
        button = new MenuButton("TP to Stilt House", "");
        button.Activated += (sender, args) => this.Telep(new Vector3(1020.5f, 663.41f, 154.75f));
        menuItems.Add(button);
        button = new MenuButton("Load Lifeinvader", "");
        button.Activated += (sender, args) => this.loadfaceinvader();
        menuItems.Add(button);
        button = new MenuButton("TP to Lifeinvader", "");
        button.Activated += (sender, args) => this.Telep(new Vector3(-1047.9f, -233.0f, 39.0f));
        menuItems.Add(button);
        button = new MenuButton("Open Page 2", "");
        button.Activated += (sender, args) => this.OpenIPL2();
        menuItems.Add(button);
        this.View.AddMenu(new Menu(("IPL Loader Menu"), menuItems.ToArray()));
    }
    private void OpenIPL2()
    {
        var menuItems = new List<IMenuItem>();
        var button = new MenuButton("Load FIB", "");
        button.Activated += (sender, args) => this.loadfib();
        menuItems.Add(button);
        button = new MenuButton("TP to FIB", "");
        button.Activated += (sender, args) => this.Telep(new Vector3(110.4f, -744.2f, 45.7f));
        menuItems.Add(button);
        button = new MenuButton("Load Heist Carrier", "");
        button.Activated += (sender, args) => this.loadheistcarrier();
        menuItems.Add(button);
        button = new MenuButton("TP to Heist Carrier", "");
        button.Activated += (sender, args) => this.Telep(new Vector3(-3069.9f, -4632.4f, 16.2f));
        menuItems.Add(button);
        button = new MenuButton("Load Heist Yacht", "");
        button.Activated += (sender, args) => this.loadheistyacht();
        menuItems.Add(button);
        button = new MenuButton("TP to Heist Yacht", "");
        button.Activated += (sender, args) => this.Telep(new Vector3(-2045.8f, -1031.2f, 11.9f));
        menuItems.Add(button);
        button = new MenuButton("Load Jewel Store", "");
        button.Activated += (sender, args) => this.loadjewelstore();
        menuItems.Add(button);
        button = new MenuButton("TP to Jewel Store", "");
        button.Activated += (sender, args) => this.Telep(new Vector3(-630.4f, -236.7f, 40.0f));
        menuItems.Add(button);
        button = new MenuButton("Load Morgue", "");
        button.Activated += (sender, args) => this.loadmorgue();
        menuItems.Add(button);
        button = new MenuButton("TP to Morgue", "");
        button.Activated += (sender, args) => this.Telep(new Vector3(244.9f, -1374.7f, 39.5f));
        menuItems.Add(button);
        button = new MenuButton("Load Train Crash", "");
        button.Activated += (sender, args) => this.loadtraincrash();
        menuItems.Add(button);
        button = new MenuButton("TP Train Crash", "");
        button.Activated += (sender, args) => this.Telep(new Vector3(-532.1309f, 4526.187f, 88.7955f));
        menuItems.Add(button);
        button = new MenuButton("Load UFO(s)", "");
        button.Activated += (sender, args) => this.loadtraincrash();
        menuItems.Add(button);
        button = new MenuButton("TP To UFO 1", "Over fort zancudo");
        button.Activated += (sender, args) => this.Telep(new Vector3(-2051.99463f, 3237.05835f, 1456.97021f));
        menuItems.Add(button);
        button = new MenuButton("TP To UFO 2", "Over stoner camp");
        button.Activated += (sender, args) => this.Telep(new Vector3(2490.47729f, 3774.84351f, 2414.035f));
        menuItems.Add(button);
        this.View.AddMenu(new Menu(("IPL Loader Menu 2"), menuItems.ToArray()));
    }
    ...
    private void OpenCommercialSpawnMenu()
    {
        ...
        button = new MenuButton("Trailers Menu", "");
    ...
    private void OpenSpawnMenuVehicle()
    {
        ...
        button = new MenuButton("Commercial Vehicles & Trailers", "");
        ...
        if (isontestmode)
        {
            button = new MenuButton("Settings", "");
            button.Activated += (sender, args) => this.spawnsettings();
            menuItems.Add(button);
        }
    ...
    {
        //return Function.Call<Vehicle>(Hash.CREATE_VEHICLE, vehname, pos.X, pos.Y, pos.Z, heading, nethandle, vehhandle);
        return World.CreateVehicle(((Model)vehname), pos, heading);
    }
    ...
    void NewSpawnBodyguard(Ped player, bool invincible, bool nsfw)
    {
        int playerGroup = Function.Call<int>(Hash.GET_PED_GROUP_INDEX, player);

        Random r = new Random();
        Vector3 spawnLoc = player.Position + new Vector3((r.Next(0, 30) / 10), (r.Next(0, 30) / 10), 0);

        List<int> nsfwmodel_names = new List<int>();
        List<string> sfwmodel_names = new List<string>();
        nsfwmodel_names.Add(1943971979);
        nsfwmodel_names.Add(-287649847);
        nsfwmodel_names.Add(-835930287);
        nsfwmodel_names.Add(-1661836925);
        nsfwmodel_names.Add(2109968527);
        nsfwmodel_names.Add(-1251702741);
        //Still 2sfw4u
        sfwmodel_names.Add("s_f_y_stripper_01");
        sfwmodel_names.Add("s_f_y_hooker_01");
        sfwmodel_names.Add("s_f_y_hooker_02");
        sfwmodel_names.Add("s_f_y_hooker_03");
        sfwmodel_names.Add("csb_stripper_01");
        sfwmodel_names.Add("s_f_y_stripper_01");
        sfwmodel_names.Add("csb_stripper_02");
        sfwmodel_names.Add("s_f_y_stripper_02");
        sfwmodel_names.Add("s_f_y_stripperlite");

        Ped peds = Game.Player.Character;
        if (nsfw)
        {
            peds = World.CreatePed(nsfwmodel_names[r.Next(0, nsfwmodel_names.Count)], spawnLoc);
        }
        else
        {
            peds = World.CreatePed(sfwmodel_names[r.Next(0, sfwmodel_names.Count)], spawnLoc);
        }

        foreach (WeaponHash wpn in Enum.GetValues(typeof(WeaponHash)))
        {
            peds.Weapons.Give(wpn, 9999, false, true);
        }

        peds.Task.ClearAllImmediately();
        peds.Task.GoTo(player);
        peds.CanSwitchWeapons = true;
        peds.CanRagdoll = false;
        peds.Health = peds.MaxHealth;
        peds.Armor = peds.MaxHealth;
        peds.IsInvincible = invincible;

        Function.Call(Hash.SET_PED_AS_GROUP_MEMBER, playerGroup);
    }
    ...
    void loadtraincrash()
    {
        Function.Call(Hash.REQUEST_IPL, "canyonriver01_traincrash");
        Function.Call(Hash.REQUEST_IPL, "railing_end");
        Function.Call(Hash.REMOVE_IPL, "railing_start");
        Function.Call(Hash.REMOVE_IPL, "canyonriver01");
    }

    void loadstilthouse()
    {
        Function.Call(Hash.REQUEST_IPL, "DES_StiltHouse_imapend");
        Function.Call(Hash.REMOVE_IPL, "DES_StiltHouse_imapstart");
        Function.Call(Hash.REMOVE_IPL, "des_stilthouse_rebuild");
    }

    void loadplanecrash()
    {
        Function.Call(Hash.REQUEST_IPL, "Plane_crash_trench");
        Prop plane = World.CreateProp("prop_shamal_crash", new Vector3(2808.387f, 4795.484f, 47.2104f), false, false);
        plane.Rotation = new Vector3(-1.2048f, -6.1718f, -12.3839f);
    }

    void loadcluckingbell()
    {
        Function.Call(Hash.REMOVE_IPL, "CS1_02_cf_offmission"); // remove the doors so you can enter
    ...
    private void DoUnlockAllWeapons(Ped playa)
    {
        foreach (WeaponHash wpn in Enum.GetValues(typeof(WeaponHash)))
        {
            playa.Weapons.Give(wpn, 9999, false, true);
        }
    }

    private void DoRemoveAllWeapons(Ped playa)
    {
        playa.Weapons.RemoveAll();
    }
